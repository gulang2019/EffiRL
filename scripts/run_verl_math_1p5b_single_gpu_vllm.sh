...
GSM8K_TRAIN="${GSM8K_TRAIN:-${ROOT_DIR}/data/verl/gsm8k/train.parquet}"
GSM8K_TEST="${GSM8K_TEST:-${ROOT_DIR}/data/verl/gsm8k/test.parquet}"
MATH_TRAIN="${MATH_TRAIN:-${ROOT_DIR}/data/verl/math/train.parquet}"
...
missing=0
for path in "${GSM8K_TRAIN}" "${GSM8K_TEST}" "${MATH_TRAIN}" "${MATH_TEST}"; do
  if [[ ! -f "${path}" ]]; then
...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-qwen2.5-1.5b-grpo-math-single-gpu-vllm}"
...
if [[ -n "${TRAIN_FILES:-}" ]]; then
  TRAIN_FILES_VALUE="${TRAIN_FILES}"
else
  TRAIN_FILES_VALUE="['${GSM8K_TRAIN}', '${MATH_TRAIN}']"
fi

if [[ -n "${VAL_FILES:-}" ]]; then
  VAL_FILES_VALUE="${VAL_FILES}"
elif [[ "${VAL_SUBSET_SIZE}" -gt 0 ]]; then
  ...
  VAL_FILES_VALUE="['${VAL_SUBSET_FILE}']"
else
  VAL_FILES_VALUE="['${GSM8K_TEST}', '${MATH_TEST}']"
fi
...
"${PYTHON_BIN}" -m verl.trainer.main_ppo \
  ...
  data.train_files="${TRAIN_FILES_VALUE}" \
  data.val_files="${VAL_FILES_VALUE}" \
